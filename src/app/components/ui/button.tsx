...
const Button: FC<ButtonProps> = ({ customType, extraClass, ...props }) => {
  const buttonclasses = {
    ...
    outline: "bg-transparent border-rose-500 active:bg-rose-200 text-rose-900",
  ...
  return (
    <button
      {...props}
      className={`w-full rounded flex items-center gap-1 justify-center active:scale-95 transition-all  font-semibold text-sm p-2 ${buttonClass} ${extraClass}`}
    ></button>
  );
};
